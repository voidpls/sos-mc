...
module.exports.run = async (bot, msg, args) => {
    try {
        ...
        let res = await Promise.all([
            axios.get(`https://api.ipgeolocation.io/ipgeo?apiKey=${IP_API_KEY}&ip=${SERVER_IP}`),
        ...
        const geoInfo = res[0].data ? `${res[0].data.state_prov}, ${res[0].data.country_code2}` : 'N/A'
        const playerNum =
            `${res[1].match(/are (\d+) of/m)[1]}/${res[1].match(/max of (\d+) players/m)[1]}`
        const version = res[2].match(/running (.+) version .+ \(MC: ([\d\.]+)\)/m)
            .slice(1).join(' ')
        ...
        const embed = new MessageEmbed()
            .setColor(BOT_COLOR)
            .setAuthor(bot.user.username, bot.user.avatarURL({format: 'png', size: 128}))
            .setDescription(
                `**\`Version\`** \n${version}\n` +
                `**\`Region\`** \n${geoInfo}\n` +
                `**\`Players\`** \n${playerNum}\n` +
    ...
    } catch (e) { bot.cmdFailed(msg); console.log(e) }
}
